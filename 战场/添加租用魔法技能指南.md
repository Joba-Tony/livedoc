...
public:
    ...
    int m_timeRemoveHanJian;    // 移除寒箭埋骨魔法的年月日  ← 新增
};
```
...
```cpp
// 大约在 line 18970 附近
else if(strcmp(m_pItemBorrowConfigList[iBorrowItemIndex]->cGotItemName, "寒箭埋骨") == 0)
{
    // 1. 检查是否已经学会
    if(m_pClientList[iClientH]->m_cMagicMastery[97] != 0)
    {
        ShowClientMsg(iClientH, "你原本就已经学会了寒箭埋骨魔法.");
        return;
    }
    ...
    // 5. 显示租用成功提示
    sprintf(G_cTxt, "租用寒箭埋骨魔法成功！到期时间：20%d年%d月%d日",
...
{
    sprintf(G_cTxt, "您的寒箭埋骨使用到期时间为:20%d年%d月%d日",
...
| 寒箭埋骨 | 98 | 97 | m_timeRemoveHanJian | RmvHJDate |
...
- **2025-11-20**：初始版本，基于寒箭埋骨魔法租用功能实现
