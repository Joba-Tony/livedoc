...
#### 防具属性限制 (适用于头盔、铠甲/长袍、锁甲、护腿):
当装备位置为身体部位（头、身、手、腿）时，可通过 E4 和 E5 设置属性限制：

- **EffectValue4 (E4)**: 限制的属性类型
- **EffectValue5 (E5)**: 限制的数值

| E4值 | 属性类型 | 说明 |
|-----|---------|------|
| 0 | 无 | 不限制任何属性 |
| 10 | 力量 (STR) | 角色力量必须 >= E5 才能装备 |
| 11 | 敏捷 (DEX) | 角色敏捷必须 >= E5 才能装备 |
| 12 | 体质 (VIT) | 角色体质必须 >= E5 才能装备 |
| 13 | 智力 (INT) | 角色智力必须 >= E5 才能装备 |
| 14 | 魔力 (MAG) | 角色魔力必须 >= E5 才能装备 |
| 15 | 魅力 (CHR) | 角色魅力必须 >= E5 才能装备 |
...
**注意事项：**
1. E4 和 E5 必须同时设置才能生效
2. 只设置 E5 而 E4 为 0 时，限制不会生效
3. 此限制仅对身体部位装备有效（头盔、铠甲、锁甲、护腿）
4. 当角色属性低于限制值时，装备会被自动脱下
...
### 问题：WorldServer启动失败 "Init fail - Missing required .cfg files!"

这个错误信息具有**误导性**，实际上并不一定是文件缺失，更常见的原因是**配置文件解析失败**。

#### 排查步骤

**1. 检查ID是否跨文件重复**

物品ID必须在所有配置文件中**全局唯一**。WorldServer会依次读取：
- `Item.cfg`
- `Item2.cfg`
- `Item3.cfg`
- `Item4.cfg`

如果在任意文件中发现重复ID，会立即返回FALSE导致初始化失败。

**检查命令 (PowerShell)：**
```powershell
# 检查指定ID范围是否有冲突
$checkIds = 294..309  # 修改为你要检查的ID范围
@("Item.cfg","Item2.cfg","Item3.cfg","Item4.cfg") | ForEach-Object { 
    $file = $_
    $content = Get-Content "d:\GitProject\Server\WorldServer\$_" -Encoding Default
    $checkIds | ForEach-Object { 
        $id = $_
        $match = $content | Where-Object { $_ -match "Item\s*=\s*$id\s" }
        if ($match) { Write-Host "ID $id 在 $file 中存在!" -ForegroundColor Red }
    }
}
```
...
@("Item.cfg","Item2.cfg","Item3.cfg","Item4.cfg") | ForEach-Object { 
    $content = Get-Content "d:\GitProject\Server\WorldServer\$_" -Encoding Default
    $content | Where-Object { $_ -match "^Item\s*=\s*(\d+)" } | ForEach-Object { 
        if ($_ -match "^Item\s*=\s*(\d+)") { $allIds += [int]$matches[1] }
    }
}
$allIds = $allIds | Sort-Object -Unique
for ($start = 1; $start -lt 1500; $start++) { 
    $range = $start..($start+15)
    if (($range | Where-Object { $_ -notin $allIds }).Count -eq 16) { 
        Write-Host "可用范围: $start - $($start+15)"
        break 
    }
}
```
...
配置文件必须是 **ANSI (GBK)** 编码，不能是 UTF-8。

**3. 检查结束标记**

每个配置文件末尾必须有 `[ENDITEMLIST]` 标记。

**4. 检查列数**

每行必须有完整的31列（包括 `Item` 和 `=`），可用以下命令检查：
```powershell
Get-Content "Item.cfg" -Encoding Default | ForEach-Object { 
    $cols = ($_.Trim() -split '\s+').Count
    if ($_ -match "^Item" -and $cols -ne 31) { 
        Write-Host "列数错误[$cols]: $_" 
    }
}
```
...
ID重复检测逻辑位于 [worldlog.cpp#L3968-L3975](WorldServer/worldlog.cpp#L3968-L3975)：
```cpp
iItemH = atoi(token);
if(m_pItemList[iItemH] != NULL)
{
    ...
    delete cp;
    delete pStrTok;
    return FALSE;  // 这里返回FALSE导致"Missing required .cfg files"错误
}
```
